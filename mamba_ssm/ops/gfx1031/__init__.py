...
GFX1031_MANIFEST = {
    ...
    "mamba_chunk_scan_combined": {
        ...
        "selected": "upstream_triton",
        "forward_status": "pass",
        "backward_status": "pass",
        "gfx1030_status": "pass — upstream pure Triton kernel works on gfx1030 in FP16. Direct test: basic, with D/z/dt_bias, with return_final_states all pass. Backward passes via Mamba2 end-to-end (Tier 0). Triton JIT first-run compilation is slow (~5min) but cached afterward.",
        "evidence": [
            {"timestamp": "2026-07-11T16:00:00", "result": "pass", "detail": "Direct test: mamba_chunk_scan_combined(x,dt,A,B,C,chunk_size=32) shape=[1,64,4,64] fp16 on gfx1030. With D, z, dt_bias, dt_softplus=True: pass. With return_final_states: pass (final_states=[1,4,64,16]). Backward via Mamba2 Tier 0 end-to-end: pass (8/8 grads)."},
